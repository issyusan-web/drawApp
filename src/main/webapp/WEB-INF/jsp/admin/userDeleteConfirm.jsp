<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<!DOCTYPE html>
<html>

<head>
<meta charset="UTF-8">
<title>削除確認画面</title>
<%@ include file="/WEB-INF/jsp/common/header.jsp"%>
</head>

<body>
	<h1 class="headline">会員情報削除確認画面</h1>
	<p style="text-align: right">${user.name}様ログイン中</p>
	<h2 class="center">下記の内容でよろしければ、削除ボタンを押してください。</h2>
	<!-- 削除失敗時のエラーメッセージ表示 -->
	<c:if test="${not empty deleteError}">
		<div style="color: red;">
			<p>${deleteError}</p>
		</div>
	</c:if>
	<form action="${pageContext.request.contextPath}/UserDeleteController"
		method="post">
		<div class="center">
			<table border="1">
				<tr>
					<th>ユーザーID</th>
					<td>${target.loginId}</td>
				</tr>
				<tr>
					<th>お名前</th>
					<td>${target.name}</td>
				</tr>
				<tr>
					<th>パスワード</th>
					<td>${target.password}</td>
				</tr>
			</table>
		</div>
		<p class="center">
			<input type="hidden" name="userId" value="${target.id}"> <input
				type="submit" value="削除する">
		</p>
	</form>
	<p class="center">
		<a class="returnadmin" href="${pageContext.request.contextPath}/admin">管理ページに戻る</a>
	</p>
</body>

</html>